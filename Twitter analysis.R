...
# selected_twitter_data <- selected_twitter_data[selected_twitter_data$created_at > '2022-08-30',]
# 
selected_twitter_data <- filtered_data_20220630
...
for (row_num in 1:nrow(selected_twitter_data)){
  ...
  politicians <- append(politicians, toString(unlist(single_tweet['Politician_name'])))
}
...
for (name_politician in unique_politicians){
  ...
  for (row_num in 1:nrow(selected_twitter_data)){
    if (selected_twitter_data[row_num,'Politician_name'] == name_politician){
      if (!is.na(selected_twitter_data[row_num,'hashtags']) & selected_twitter_data[row_num,'hashtags'] != 'NaN' ){
        list_hashtags_single <- append(list_hashtags_single, unlist(strsplit(as.character(selected_twitter_data[row_num,'hashtags']), ","))) 
      }
    }
  }
...
# ##### START testing
# splitted <- strsplit(selected_twitter_data[row_num,'hashtags'], ",")
# 
# 
# ##### 
# unlisted_hashtags <- unlist(selected_twitter_data[row_num,'hashtags'])
# list_with_hashtags <- strsplit(unlisted_hashtags, ",")
# what <- unlist(list_with_hashtags)[1]
# ###### STOP testing
...
matrix_4_months <- adjacency.matrix
